...
def build_single_reel_prompt(
    *,
    ...
    sections.append(
        ...
        f"חובה: בחר הוק מהרשימה למטה ושנה את הנושא לנישה של {client_name}: {niche}.\n"
        f"דוגמאות להתאמה לנישה:\n"
        f"- מקורי: \"דברים שאני חושבת שהם רגילים אבל אנשים חושבים שזה קרינג׳\"\n"
        f"  → בנישת {niche}: \"דברים שאני עושה ב{niche} שאנשים חושבים שזה מוגזם\"\n"
        f"- מקורי: \"3 דברים שאני עושה כל בוקר שרוב האנשים מדלגים עליהם\"\n"
        f"  → בנישת {niche}: \"3 דברים שאני עושה כל בוקר לפני שאני פותחת את העסק\"\n"
        f"- מקורי: \"תוך 2-3 שנים מהיום את יכולה להיות במקום אחר לגמרי\"\n"
        f"  → בנישת {niche}: \"תוך 2-3 חודשים ה{niche} שלך יכול להיראות אחרת לגמרי\"\n\n"
        f"הכלל: שמור על המבנה והקצב של ההוק המקורי, שנה רק את הנושא ל-{niche}."
    )
