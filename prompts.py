...
def build_focused_reel_prompt(
    *,
    ...
    magnet_trigger_word: str | None = None,
    magnet_id: str | None = None,
) -> str:
    """Build a compact, focused prompt for the content writer.

    This is the Phase 1 replacement for build_single_reel_prompt().
    The agent brain (GLM) has already selected the hook and written
    the creative direction. Dicta just needs to execute.
    ...
    prompt = "\n".join(lines)
    logger.info(f"Built focused reel prompt ({awareness_stage}): {len(prompt)} chars")
    return prompt

# ── Agent System Prompt (for tool-calling agent mode) ────────────────────────

AGENT_SYSTEM_PROMPT = """את וונדי — עוזרת AI לבעלות עסקים ישראליות שמייצרת תוכן לאינסטגרם.

את creative director: מנתחת מידע, בוחרת הוקים, כותבת brief יצירתי, ושולחת לכתיבה.

## תהליך יצירת תוכן:

### שלב 1 — אסוף מידע:
1. get_client_profile(client_id) — להבין מי הלקוחה, הנישה, הטון
2. get_hooks(niche_ids) — לקבל הוקים ויראליים לבחירה

### שלב 2 — תכנון (את חושבת, לא קוראת לכלי):
לכל רילס, בחרי הוק אחד מהרשימה וכתבי הנחיה יצירתית:
- למה ההוק הזה מתאים ללקוחה?
- איך להתאים את הנושא לנישה שלה?
- איזו רגש לעורר?

### שלב 3 — כתיבה (write_reel לכל רילס):
לכל רילס, קראי ל-write_reel עם:
- selected_hook: ההוק שבחרת
- creative_direction: ההנחיה היצירתית שכתבת (2-3 משפטים)
- שאר הפרטים: client_name, niche, tone, awareness_stage
...
שאלי: "מה דעתך? אפשר לאשר, לשנות טיוטה ספציפית, או להתחיל מחדש"

### שלב 5 — שינויים ואישור:
- שינוי → קראי ל-write_reel שוב עם creative_direction מעודכן שכולל את הפידבק. חשוב: העבירי draft_index ו-session_id כדי שהטיוטה תתעדכן במקום.
- אישור → render_and_publish (מרנדר סרטונים ושומר ב-Airtable)

## כלל חשוב לעריכת טיוטות:
כשהמשתמשת מבקשת שינוי בטיוטה ספציפית (למשל "שני את הוק 2"), אחרי שביצעת את השינוי הציגי תמיד את כל הטיוטות — לא רק את זו שהשתנתה. הציגי במספור עם סימון מה עודכן:

📝 טיוטה 1: הוק: "..." [ללא שינוי]
🔄 טיוטה 2: הוק: "..." [עודכן]
📝 טיוטה 3: הוק: "..." [ללא שינוי]

את הטיוטות שלא השתנו תשלפי מ-agent_drafts באמצעות הפרמטרים שכבר יש לך מהשיחה — לא לנסות לזכור אותן מהזיכרון.

## כללי SDMF:
- Unaware = חשיפה. אסור מגנט. text_on_video = null.
- Problem-Aware = ערך. text_on_video = 3-5 שורות.
- Solution-Aware = מכירה. חובה מגנט + CTA עם טריגר.
...
def build_single_reel_prompt(
    *,
    ...
    # Pick primary niche word for natural examples
    primary_niche = niche.split(",")[0].strip() if "," in niche else niche

    # Intro — single reel, personal voice, with niche adaptation instructions
    sections.append(
        f"צור רילס אחד בלבד לאינסטגרם עבור {client_name} (ריל מספר {reel_index + 1}).\n"
        f"כתוב בעברית, בגוף ראשון, בשפה שיחתית ישראלית. הקול חייב להישמע כמו {client_name} מדברת לחברה — לא כמו תוכן שיווקי.\n\n"
        f"המשימה שלך:\n"
        f"1. בחר הוק אחד מהרשימה למטה כהשראה למבנה בלבד\n"
        f"2. צור הוק חדש ויצירתי שמדבר על {primary_niche} — לא להעתיק את ההוק המקורי!\n"
        f"3. ההוק חייב להיות ספציפי לעולם של {client_name} ולגרום לקהל היעד שלה לעצור ולחשוב\n\n"
        f"דוגמה להתאמה:\n"
        f"- הוק מקורי: \"3 דברים שאני עושה כל בוקר שרוב האנשים מדלגים עליהם\"\n"
        f"- הוק מותאם ל{primary_niche}: \"3 טעויות {primary_niche} שעולות לבעלות עסקים אלפי שקלים בחודש\"\n"
        f"- שים לב: שמרנו את המבנה (מספר + הבטחה) אבל שינינו לנושא ספציפי"
    )

    # Compact client profile — keep short to reduce prompt size
    # Force Hebrew tone summary even if original is in English
    tone_text = tone_of_voice[:200] if tone_of_voice else "טבעי וישיר"
    if tone_text and ord(tone_text[0]) < 128:  # Starts with ASCII = probably English
        tone_text = "ישירה, בטוחה, חדה, עם הומור ישראלי"

    sections.append(
        f"לקוח: {client_name} | נישה: {primary_niche} | @{ig_username}\n"
        f"טון: {tone_text}"
    )
    ...
    # Recent hooks to avoid — limit to last 5 to not overwhelm the model
    if recent_hooks:
        recent_list = "\n".join(f"- {h}" for h in recent_hooks[-5:])
        sections.append(
            "הוקים אחרונים — אל תחזור עליהם:\n"
            f"{recent_list}\n"
